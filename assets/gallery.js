(() => {
  ...
  slideshowForm.addEventListener('submit',async event=>{event.preventDefault();const file=slideshowInput.files[0];if(file){slideshowBackgroundUrl=URL.createObjectURL(file);slideshow.style.setProperty('--slideshow-background',`url("${slideshowBackgroundUrl}")`);}slideshow.querySelector('h2').textContent=metadataSource?.dataset.eventTitle||document.querySelector('.dash-head h1')?.textContent||'POVents Event';slideshowSetup.hidden=true;slideshow.hidden=false;slideshowIndex=0;slideshowPhotoUrls=links.map(link=>link.href);slideshowPreloaded.clear();slideshowPhotos.replaceChildren();preloadSlideshowBatch(0,6);try{if(slideshow.requestFullscreen)await slideshow.requestFullscreen();}catch(_){}await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));if(slideshow.hidden)return;renderSlideshow();startSlideshowTimer();startSlideshowPolling();});

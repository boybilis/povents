(() => {
  ...
  function startAlbumProgress(){clearInterval(albumProgressTimer);renderAlbumProgress(0);albumProgress.hidden=false;albumProgressTimer=setInterval(()=>{if(albumProgressPercent>=92)return;const step=Math.max(1,Math.ceil((92-albumProgressPercent)*.09));renderAlbumProgress(Math.min(92,albumProgressPercent+step));},320);}
  ...
  albumForm.addEventListener('submit',async event=>{event.preventDefault();albumSubmit.disabled=true;albumCancel.disabled=true;albumSubmit.textContent='Creating…';albumError.hidden=true;startAlbumProgress();const formData=new FormData(albumForm);formData.append('csrf',csrf);try{const response=await fetch(`?action=download_photo_album&event_id=${encodeURIComponent(eventId||'')}`,{method:'POST',body:formData});if(!response.ok)throw new Error(await response.text()||'The photo album could not be created.');const blob=await response.blob();renderAlbumProgress(100);await new Promise(resolve=>setTimeout(resolve,350));const disposition=response.headers.get('Content-Disposition')||'';const match=disposition.match(/filename="?([^";]+)"?/i);const downloadUrl=URL.createObjectURL(blob);const anchor=document.createElement('a');anchor.href=downloadUrl;anchor.download=match?.[1]||'POVents-photo-album.html';document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(downloadUrl),1000);hideAlbumProgress();closeAlbumModal();window.POVentsToast?.('Photo album created and downloaded.', 'success');}catch(error){hideAlbumProgress();albumError.textContent=error.message||'The photo album could not be created.';albumError.hidden=false;window.POVentsToast?.(error.message||'The photo album could not be created.','error');}finally{albumSubmit.disabled=false;albumCancel.disabled=false;albumSubmit.textContent='Create and download';}});

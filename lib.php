...
function download_photo_album(array $event, bool $shared = false, ?string $coverDataUri = null): never {
    ...
    echo '<style>.page{padding:0;border:0;border-radius:0;box-shadow:none;background:transparent}.page-surface{position:absolute;inset:0;overflow:hidden;padding:clamp(14px,3vw,38px);background:#f7f1e4;color:#17231f;box-shadow:inset 18px 0 28px #8d806522}.cover .page-surface{display:grid;place-items:center;text-align:center;color:#fff;background:linear-gradient(145deg,#113f31,#071c16)}.page-surface>.grid{height:calc(100% - 58px)}@media(max-width:700px){.page-surface{padding:10px 10px 115px}.page-surface>.grid{height:calc(100% - 46px)}}</style>';
    echo '<article class="page cover" data-density="hard"><div class="page-surface"'.$coverStyle.'><div><img src="data:image/png;base64,'.base64_encode((string)file_get_contents(__DIR__.'/assets/povents-logo-dark.png')).'" alt="POVents"><h1>'.$safeTitle.'</h1><p>'.e($date).'</p><p>'.count($photos).' memories · available offline</p></div></div></article>';
    ...
    echo '<script>(()=>{const bookElement=document.querySelector("#book"),pages=document.querySelectorAll(".page"),prev=document.querySelector("#prev"),next=document.querySelector("#next"),full=document.querySelector("#full"),count=document.querySelector("#count");const flip=new St.PageFlip(bookElement,{width:1200,height:900,size:"stretch",minWidth:280,maxWidth:1800,minHeight:210,maxHeight:1350,drawShadow:true,flippingTime:900,usePortrait:true,startZIndex:10,autoSize:true,maxShadowOpacity:.55,showCover:true,mobileScrollSupport:false,swipeDistance:30,clickEventForward:true,useMouseEvents:true,showPageCorners:false,disableFlipByClick:true});function controls(){const i=flip.getCurrentPageIndex(),total=flip.getPageCount();prev.disabled=i===0;next.disabled=i>=total-1;count.textContent=`${i+1} / ${total}`}flip.on("init",controls);flip.on("flip",controls);flip.on("changeOrientation",controls);flip.loadFromHTML(pages);prev.onclick=()=>flip.flipPrev("bottom");next.onclick=()=>flip.flipNext("bottom");full.onclick=async()=>{try{if(document.fullscreenElement){await document.exitFullscreen();screen.orientation?.unlock?.()}else{await document.documentElement.requestFullscreen?.();await screen.orientation?.lock?.("landscape")}}catch(_){full.textContent="Rotate landscape"}};addEventListener("fullscreenchange",()=>{full.textContent=document.fullscreenElement?"Exit fullscreen":"⛶ Fullscreen";setTimeout(()=>flip.update(),100)});addEventListener("keydown",e=>{if(e.key==="ArrowLeft")flip.flipPrev("bottom");if(e.key==="ArrowRight")flip.flipNext("bottom");if(e.key.toLowerCase()==="f")full.click()});controls()})();</script></body></html>';

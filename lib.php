...
function download_photo_album(array $event): never {
    ...
    $albumPages = [];
    foreach ($orientationOrder as $orientation) foreach (array_chunk($byOrientation[$orientation], 4) as $items) $albumPages[] = ['orientation'=>$orientation,'photos'=>$items];
    ...
    echo '<style>.page{border:0;box-shadow:none}.cover{border:0}.page:not(.cover){grid-template-rows:minmax(0,1fr)}.grid{width:100%;height:100%;min-width:0;min-height:0;overflow:hidden}.photo{display:flex;align-items:center;justify-content:center;min-width:0;min-height:0;overflow:hidden;padding:0;background:transparent;box-shadow:none;transform:none}.photo img{width:100%;height:100%;max-width:100%;max-height:100%;object-fit:contain;border:3px solid #fff;border-radius:8px;background:#e7e0d2}.portrait-page .grid{grid-template-columns:repeat(4,minmax(0,1fr));grid-template-rows:minmax(0,1fr);gap:10px}.portrait-page .photo img{width:100%;height:auto;max-height:100%}.landscape-page .grid{grid-template-columns:repeat(2,minmax(0,1fr));grid-template-rows:repeat(2,minmax(0,1fr))}</style>';
    ...
    foreach ($albumPages as $pageIndex => $albumPage) {
        ...
        echo '<article class="page '.$albumPage['orientation'].'-page"><div class="grid">';

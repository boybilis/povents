...
function download_photo_album(array $event): never {
    ...
    foreach ($orientationOrder as $orientation) {
        $photosPerPage = $orientation === 'portrait' ? 4 : 6;
    ...
    echo '<style>.page{border:0;box-shadow:none}.cover{border:0}.page:not(.cover){grid-template-rows:minmax(0,1fr)}.grid{width:100%;height:100%;min-width:0;min-height:0;overflow:hidden}.photo{display:flex;align-items:center;justify-content:center;min-width:0;min-height:0;overflow:hidden;padding:0;background:transparent;box-shadow:none;transform:none}.photo img{width:auto;height:auto;max-width:100%;max-height:100%;object-fit:contain;border:3px solid #fff;border-radius:8px;background:transparent}.portrait-page .grid{grid-template-columns:repeat(4,minmax(0,1fr));grid-template-rows:minmax(0,1fr);gap:10px}.landscape-page .grid{grid-template-columns:repeat(3,minmax(0,1fr));grid-template-rows:repeat(2,minmax(0,1fr));gap:10px}</style>';

...
ob_start(static function (string $html): string {
    ...
    if (str_contains($html, 'id="guest-link"') && isset($_GET['id'])) {
        $downloadButton = '<p class="event-downloads"><a class="button light" href="?action=download_event_qr&amp;event_id='.(int)$_GET['id'].'">Download branded QR image</a><a class="button light" href="?action=download_photo_album&amp;event_id='.(int)$_GET['id'].'">Download offline photo album</a></p>';
        $html = preg_replace('~(<div class="copyline">.*?</div>)~s', '$1'.$downloadButton, $html, 1) ?? $html;
    }
    return str_replace(
        ['</head>','</body>'],
        ['<link rel="icon" href="assets/povents-logo.png"><link rel="stylesheet" href="assets/responsive.css?v=6"></head>','<script src="assets/gallery.js?v=2"></script></body>'],
        $html
    );
});

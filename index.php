...
ob_start(static function (string $html): string {
    ...
    if (str_contains($html, 'id="guest-link"') && isset($_GET['token']) && ($currentUser = user())) {
        ...
        $downloadButton = '<p class="event-downloads"><button class="button light presentation-qr-create" type="button" data-event-id="'.$eventId.'" data-event-title="'.$eventTitle.'" data-event-date="'.htmlspecialchars($eventDate,ENT_QUOTES,'UTF-8').'" data-event-time="'.htmlspecialchars($eventTime,ENT_QUOTES,'UTF-8').'" data-reels-created="'.$reelsCreated.'" data-reels-allowed="'.$reelsAllowed.'" data-reel-duration="'.(int)($currentEvent['reel_duration_seconds'] ?? 30).'" data-reel-images="'.(int)($currentEvent['reel_image_count'] ?? 20).'">Create Presentation QR</button></p>';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'claim_reel') {
        ...
        $u=require_user(); $event=event_for_owner((int)($_POST['event_id']??0),(int)$u['id']);
        if(!$event){http_response_code(404);echo json_encode(['error'=>'Event not found.']);exit;}
        $claim=db()->prepare('UPDATE events SET reels_created=reels_created+1 WHERE id=? AND user_id=? AND (reels_allowed=0 OR reels_created<reels_allowed)');
        ...
        $remaining=db()->prepare('SELECT IF(reels_allowed=0,-1,GREATEST(0,reels_allowed-reels_created)) FROM events WHERE id=?');$remaining->execute([$event['id']]);
    ...
    if ($action === 'create_event') {
        ...
        $s=db()->prepare('INSERT INTO events(user_id,title,event_date,start_time,end_time,location,token,pricing_plan_id,plan_name,max_guest_scans,max_photos_per_session,photo_retention_days,reels_allowed,photo_albums_allowed,reel_duration_seconds,reel_image_count) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
        $s->execute([$u['id'],$title,$eventDate,$startTime.':00',$endTime.':00',trim($_POST['location'] ?? ''),$eventToken,$plan['id'],$plan['name'],$plan['max_guest_scans'],$plan['max_photos_per_session'],$plan['photo_retention_days'],$plan['reels_per_event'],$plan['photo_albums_per_event'],$plan['reel_duration_seconds'],$plan['reel_image_count']]);
    ...
    if ($action === 'save_plan') {
        ...
        $values=[trim((string)($_POST['description']??'')),$price,max(1,(int)($_POST['passes_per_purchase']??1)),max(0,(int)($_POST['max_guest_scans']??50)),max(1,(int)($_POST['max_photos_per_session']??5)),max(1,(int)($_POST['photo_retention_days']??7)),max(0,(int)($_POST['reels_per_event']??0)),max(0,(int)($_POST['photo_albums_per_event']??1)),max(10,(int)($_POST['reel_duration_seconds']??30)),max(2,(int)($_POST['reel_image_count']??20)),isset($_POST['is_active'])?1:0,isset($_POST['is_featured'])?1:0,max(0,(int)($_POST['display_order']??0))];
        if(strlen($name)<2||$price<100){flash('error','Enter a plan name and a valid price.');go('?page=admin-settings'.($id?'&edit='.$id:''));}
        if($id){db()->prepare('UPDATE pricing_plans SET name=?,description=?,price_centavos=?,passes_per_purchase=?,max_guest_scans=?,max_photos_per_session=?,photo_retention_days=?,reels_per_event=?,photo_albums_per_event=?,reel_duration_seconds=?,reel_image_count=?,is_active=?,is_featured=?,display_order=? WHERE id=?')->execute(array_merge([$name],$values,[$id]));}
        else{$slug=preg_replace('/[^a-z0-9]+/','-',strtolower($name)).'-'.bin2hex(random_bytes(3));db()->prepare('INSERT INTO pricing_plans(name,slug,description,price_centavos,passes_per_purchase,max_guest_scans,max_photos_per_session,photo_retention_days,reels_per_event,photo_albums_per_event,reel_duration_seconds,reel_image_count,is_active,is_featured,display_order) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute(array_merge([$name,$slug],$values));}
...
<main class="shell pricing-page"><div class="pricing-head"><div class="eyebrow">Choose your event plan</div><h1>One event. Your preferred features.</h1><p class="lead">Select the plan the administrator has made available.</p></div><?php if(!$plans): ?><div class="empty">No pricing plans are currently available.</div><?php else: ?><div class="pricing-grid"><?php foreach($plans as $plan): ?><article class="pricing-card <?=$plan['is_featured']?'featured':''?>"><?php if($plan['is_featured']): ?><span class="plan-badge">Recommended</span><?php endif; ?><h2><?=e($plan['name'])?></h2><p class="muted"><?=e($plan['description'])?></p><div class="plan-price">₱<?=number_format($plan['price_centavos']/100,2)?><small> / event pass</small></div><ul><li><?=$plan['max_guest_scans']?number_format($plan['max_guest_scans']).' guest scans':'Unlimited guest scans'?></li><li><?=$plan['max_photos_per_session']?> photos per guest scan</li><li><?=$plan['photo_retention_days']?> days original-photo storage</li><li><?=$plan['reels_per_event']?$plan['reels_per_event'].' reels':'Unlimited reels'?> · <?=$plan['reel_duration_seconds']?> seconds</li><li><?=$plan['photo_albums_per_event']?> saved photo album<?=$plan['photo_albums_per_event']==1?'':'s'?></li></ul><form method="post" action="?action=subscribe"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="plan_id" value="<?=$plan['id']?>"><button class="full"><?=local_payment_bypass()?'Add local pass':'Select & pay with QRPh'?></button></form></article><?php endforeach; ?></div><?php endif; ?></main>
